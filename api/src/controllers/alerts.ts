...
const getAllActiveAlerts = async (userData: UserData): Promise<void> => {
  ...
  try {
    ...
    for (const alert of alertList) {
      if (!mergedMap.has(alert.id)) {
        mergedMap.set(alert.id, { ...alert });
      } else {
        const existing: any = mergedMap.get(alert.id);
        const existingAreas = existing.areaDesc
          .split(";")
          .map((s: any) => s.trim());
        const newAreas = alert.areaDesc.split(";").map((s) => s.trim());

        const mergedAreas = Array.from(
          new Set([...existingAreas, ...newAreas])
        );
        existing.areaDesc = mergedAreas.join("; ");

        // You could also merge other fields here if needed (e.g., ends, headline)
        mergedMap.set(alert.id, existing);
      }
    }

    alertList = Array.from(mergedMap.values());
